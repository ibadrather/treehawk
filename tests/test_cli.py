...
def test_the_everyday_options_are_the_short_list(command):
...
def test_watch_rejects_two_ways_of_naming_the_same_thing():
    result = runner.invoke(app, ["watch", "train.py", "--pid", "42"])
    assert result.exit_code == 1
    assert "just one" in result.output


def test_watch_reports_a_process_that_is_not_running(tmp_path):
    result = runner.invoke(
        app, ["watch", "definitely-not-running-xyzzy", "-q",
...
def test_run_without_a_command_says_so():
    result = runner.invoke(app, ["run"])
    assert result.exit_code == 1
    assert "after --" in result.output


def test_an_impossible_interval_is_refused():
    result = runner.invoke(app, ["watch", "x", "-i", "0"])
    assert result.exit_code != 0


def test_report_renders_a_log(log):
...
def test_report_can_emit_json(log):
    result = runner.invoke(app, ["report", log, "--json"])
    assert result.exit_code == 0
    assert '"summary"' in result.stdout


def test_report_on_a_missing_file_fails_clearly(tmp_path):
    result = runner.invoke(app, ["report", str(tmp_path / "nope.jsonl")])
    assert result.exit_code == 1
    assert "cannot read" in result.output


def test_pdf_writes_next_to_the_log(tmp_path):
...
def test_pdf_on_an_empty_log_fails_clearly(tmp_path):
