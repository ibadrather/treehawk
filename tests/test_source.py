...
def test_scan_returns_every_process_and_ignores_non_pid_entries(proc_root: pathlib.Path) -> None:
...
def test_scan_skips_a_process_that_exits_mid_read(proc_root: pathlib.Path) -> None:
    write_proc(proc_root, 100)
    broken = write_proc(proc_root, 101)
    (broken / "stat").write_text("101 (worker) S")  # truncated, as a racing read would see

    assert set(LinuxProcessSource(str(proc_root)).scan()) == {100}


def test_enrich_adds_cmdline_pss_and_cgroup(proc_root: pathlib.Path) -> None:
    write_proc(
        proc_root,
        100,
...
def test_enrich_skips_pss_when_not_wanted(proc_root: pathlib.Path) -> None:
...
def test_enrich_tolerates_an_unreadable_process(proc_root: pathlib.Path) -> None:
...
def test_cmdline_is_cached_per_identity_and_dropped_on_exit(proc_root: pathlib.Path) -> None:
