...
def test_pids_in_includes_nested_groups(cgroup_root: pathlib.Path) -> None:
...
def test_pids_in_returns_none_for_a_group_that_is_gone(cgroup_root: pathlib.Path) -> None:
    assert CgroupV2Source(str(cgroup_root)).pids_in("/vanished.scope") is None


def test_metrics_reads_cpu_and_memory(cgroup_root: pathlib.Path) -> None:
    write_cgroup(
        cgroup_root,
        "/app.scope",
        pids=[100],
...
def test_metrics_treats_max_and_missing_files_as_unknown(cgroup_root: pathlib.Path) -> None:
    directory = write_cgroup(cgroup_root, "/app.scope", pids=[100])
    (directory / "memory.current").write_text("max\n")
...
def test_available_reflects_a_real_mount(cgroup_root: pathlib.Path, tmp_path: pathlib.Path) -> None:
