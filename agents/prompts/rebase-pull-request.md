<!-- Adapted from uv's agents/prompts/rebase-pull-request.md
     (https://github.com/astral-sh/uv), MIT OR Apache-2.0. -->
...
- Run `cargo fmt` and `cargo clippy --all-targets -- -D warnings`, fix any failures introduced by
  the rebase, and prefer focused checks over the full suite.
- Keep the changes narrowly scoped to the pull request. Do not add dependencies, run release
  builds, or make unrelated cleanups.
