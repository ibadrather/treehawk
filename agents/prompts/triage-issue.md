<!-- Adapted from uv's agents/prompts/triage-issue.md (https://github.com/astral-sh/uv),
     MIT OR Apache-2.0. -->
...
subsystems (e.g., `run`, `report`, watch mode, service mode, cgroup handling, a GPU vendor path),
triggering conditions, and exact identifiers or error fragments. Search each distinct claim
separately when an issue contains more than one.
...
kernel versions, distros, and hardware details to look for the underlying behavior. Keep searches
based on observable symptoms or requested capabilities separate from searches based on possible
causes so an assumed cause does not displace a closer result. For version-specific reports, search
closed issues and merged pull requests for related fixes.
...
over shared platforms, hardware, or terminology. Include an adjacent result only when its reason
