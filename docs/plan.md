...
**7. `treehawk report <session>` (FR-19).** Read manifest + Parquet (+ any `.arrows` tail): per-process peak/mean/p95 CPU (per-core, as in `top`), peak RSS, lifetime, exit code; session totals, overruns, duration. Plain table, color only on TTY (FR-21).
